    ...
    public class BuildMenuTool : BaseUnityPlugin
    {
        ...
        public static void InitUI()
        {
            ...
            if (GenesisCompatibility)
            {
                ...
                switchHotkeyRowText.GetComponent<Text>().fontSize = 10;
            }
        ...
        public static void UIBuildMenuOnUpdatePostPatch(ref UIFunctionPanel __instance)
        {
            ...
            if (guideComplete)
            {
                if (_this.buildMenu.active)
                {
                    ...
                    {
                        bool unlocked = forceShowAllButtons; // show the row2 only if at least one item in this category in row2 is unlocked. or when holding rebind key (showAllButtons is true).
